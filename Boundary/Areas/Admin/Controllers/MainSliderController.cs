    ...
    public class MainSliderController : BaseController
    {
        public ActionResult Index()
        {
            try
            {
                ...
                return View(new FirstPage_SliderBL().SelectAll());
            }
        ...
        public ActionResult PostCreate(FirstPage_Slider slider, HttpPostedFileBase file)
        {
            try
            {
                CheckSessionDataModel checkSession = CheckAdminSession();
                ...
                string filePath = System.Web.HttpContext.Current.Server.MapPath("~/Content/Images/Admin/FirstPage/Slider");
                ...
                slider.ImgAddress = rootPath;
                if (new FirstPage_SliderBL().Insert(slider) > 0)
                {
            ...
            catch (MyExceptionHandler exp1)
            {
                try
                {
                    List<ActionInputViewModel> lst = new List<ActionInputViewModel>()
                    {
                        ...
                        {
                            Name = HelperFunctionInBL.GetVariableName(() => slider),
                            Value = JObject.FromObject(slider).ToString()
                        },
                    };
                    long code = new ErrorLogBL().LogException(exp1, User.Identity.GetUserId() ?? Request.UserHostAddress, JArray.FromObject(lst).ToString());
                    return Json(JsonResultHelper.FailedResultWithTrackingCode(code), JsonRequestBehavior.AllowGet);
        ...
        public ActionResult Edit(long id)
        {
            try
            {
                ...
                return View(new FirstPage_SliderBL().SelectOne(id));
            }
            catch (MyExceptionHandler exp1)
            {
                try
        ...
        public ActionResult PostEdit(FirstPage_Slider slider, HttpPostedFileBase file)
        {
            try
            {
                CheckSessionDataModel checkSession = CheckAdminSession();
                ...
                if (file != null)
                {
                    ...
                    filePath = System.Web.HttpContext.Current.Server.MapPath("~/Content/Images/Admin/FirstPage/Slider");
                ...
                string lastBoxImgAddress = new FirstPage_SliderBL().GetImgAddressById(slider.Id);

                slider.ImgAddress = (string.IsNullOrEmpty(rootPath)) ? lastBoxImgAddress : rootPath;

                bool result = new FirstPage_SliderBL().Update(slider);
        ...
        public ActionResult PostDelete(long id)
        {
            try
            {
                ...
                string imgAddress = new FirstPage_SliderBL().GetImgAddressById(id);
                if (new FirstPage_SliderBL().Delete(id))
                {
